---
title: 1111 人力銀行形象活動網站
description: 為 1111 人力銀行打造的形象活動網站，結合互動彈窗與 Hover 動態效果，強化品牌形象與使用者體驗。
date: "2022"
category: Web Design
tags: [RWD, JavaScript]
cover: /images/portfolio/1111-event-site/cover.png
featured: true
order: 1
externalLink: https://hsiaohan.myportfolio.com/web-designevent-image-site-of-1111-job-bank
---

1111 人力銀行的形象活動網站，網站中設計了多種互動細節：案例項目的 Hover 狀態切換呈現警示風格效果；討論區頁面透過 Call to Action 按鈕開啟彈窗，方便使用者快速發問；登入彈窗依不同表單類型切換顯示；分享按鈕與管理員權限選單皆採用收合式互動設計。

![設計主視覺](/images/portfolio/1111-event-site/01.png)

![案例項目互動設計](/images/portfolio/1111-event-site/02.png)
